...
export class AnalyticsService {
  ...
  getSubscriptionsByStatus(): Observable<ChartData> {
    return this.http.get<ApiSuccessResponse<ChartData>>(`${this.apiUrl}/subscriptions-by-status`).pipe(
  ...
  getNotificationsSentVsBilled(): Observable<ChartData> {
    return this.http.get<ApiSuccessResponse<ChartData>>(`${this.apiUrl}/notifications-sent-vs-billed`).pipe(
      map(response => response.data)
    );
  }
  ...
  getTopWorkflows(limit: number = 10): Observable<ChartData> {
    ...
    return this.http.get<ApiSuccessResponse<ChartData>>(`${this.apiUrl}/workflows/top`, { params }).pipe(
      map(response => response.data)
    );
  }
  ...
  getUsageByStatus(): Observable<UsageByStatusResponse> {
    return this.http.get<ApiSuccessResponse<UsageByStatusResponse>>(`${this.apiUrl}/usage/by-subscription-status`).pipe(
      map(response => response.data)
    );
  }
}